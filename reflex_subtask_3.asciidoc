...
////
It's hard to see for sure, but in the photo, it looks like it's plugged into pin 5 on the ATmega chip (row 13).
////
...
Also verify that the other jumper wire leaving the pushbutton is sharing a row with RES2 and JUMP4 on one end, and in inserted into pin 6 on the ATmega chip on the other end.
====
